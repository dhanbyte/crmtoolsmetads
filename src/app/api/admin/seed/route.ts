...
export async function GET() {
  try {
    const { count, error: countError } = await supabaseAdmin.from('leads').select('*', { count: 'exact', head: true });
    
    // Ignore error if it's just about the table not existing yet or something similar
    // but here we expect the table to exist.
    
    if (count && count > 0) {
      return NextResponse.json({ message: `Database already has ${count} leads. Skipping seed.` });
    }
    ...
    if (insertError) {
      ...
      return NextResponse.json({ message: 'Successfully seeded leads (without interest column fallback)' });
    }

    return NextResponse.json({ message: 'Successfully seeded 25 mock leads!' });
  } catch (error: any) {
